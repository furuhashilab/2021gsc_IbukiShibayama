...
ドローンの測量は、古橋大地教授の主導の下2021年12月15日にエアロセンス社製エアロボウイングを用いて行った。そこで得られたLASデータをCoudCompareでサイズ調整とトリミングを行ってPLYファイルにエクスポートし、Blenderで点群に沿うようにメッシュの3Dモデルを作成する。このモデルをOBJやFBXなどのファイルにエクスポートし、GitHubやSketchfabなどでオープンデータとして公開する。なお、CloudCompareとBlenderを選択したのは、無料ソフトウェアであり、より多くのユーザーが最低限のコストで利用できると考えたからである。
## Results
